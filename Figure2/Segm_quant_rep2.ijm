folder = "/home/sasha/Science/PhD/Laue_lab/Microscopy/Confocal/Histone_marks/20231009_HP1b-H-646_DNA-505_H3K9me3-Cy3_MacheskyConfocal/";
for (f = 1; f < 8; f++) {
	//f=1;
	open(folder + "Pos" + f + "_confocal_z-stack_deconv.czi");
	waitForUser; //duplicate the z-slice through the middle
	rename("Pos" + f + "_confocal_z-stack_deconv.czi");
	run("Split Channels");
	selectImage("C1-Pos" + f + "_confocal_z-stack_deconv.czi");
	//run("Z Project...", "projection=[Max Intensity]");
	run("Grays");
	rename("HP1");
//	selectImage("C1-Pos" + f + "_confocal_z-stack_deconv.czi");
//	close();
	selectImage("C2-Pos" + f + "_confocal_z-stack_deconv.czi");
	//run("Z Project...", "projection=[Max Intensity]");
	run("Grays");
	rename("H3K9me3");
//	selectImage("C2-Pos" + f + "_confocal_z-stack_deconv.czi");
//	close();
	selectImage("C3-Pos" + f + "_confocal_z-stack_deconv.czi");
	//run("Z Project...", "projection=[Max Intensity]");
	run("Grays");
	rename("DNA");
//	selectImage("C3-Pos" + f + "_confocal_z-stack_deconv.czi");
...
//			run("Analyze Particles...", "size=12-Infinity pixel show=Masks clear");
//			rename("HP1_mask" + j);
//			run("Invert");
//			waitForUser;
	}
	...
	run("Analyze Particles...", "size=6-Infinity pixel show=Masks clear");
	rename("HP1_mask");
	run("Invert");
	waitForUser;
	...
	for (j=0; j<nROIs; j++){
		...
		roiManager("Select", j);
		run("Analyze Particles...", "size=12-Infinity pixel display summarize");
	}
	...
	for (j=0; j<nROIs; j++){
		...
		roiManager("Select", j);
		run("Analyze Particles...", "size=6-Infinity pixel display summarize");
	}
	selectWindow("Summary");
...
////////////////////////////////////////
//Correction: due to outliers, better to calc avg DNA and then divide avg H3K9me3 by avg DNA,
...
//	close();
	...
	for (j=0; j<nROIs; j++){
		selectWindow("DNA_mask");
		roiManager("Select", j);
		run("Analyze Particles...", "size=12-Infinity pixel display");
	}
	selectWindow("Results");
	...
	for (j=0; j<nROIs; j++){
		selectWindow("HP1_mask");
		roiManager("Select", j);
		run("Analyze Particles...", "size=6-Infinity pixel display");
	}
	selectWindow("Results");
