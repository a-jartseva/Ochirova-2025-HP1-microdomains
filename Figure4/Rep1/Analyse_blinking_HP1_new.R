...
#Bugs - fixed:
{
####NB! Found an issue: probably some repeated frames towards the end -> completely stationary molecules.
#Filtered out frames > 39981.

####NB! Found an error: for DBSCAN density estimation, should use mean nearest neighbour distance,
# not mean of all distances -> changed, also changed the coefficient.

####NB! Found an issue in DNA segmentation: somehow 1 focus per position is missed in the saved foci. Found the bug, now ok.

####In dbscan, dist = 'euclidean' changed to metric = 'euclidean'
...
folder = '/home/sasha/Science/PhD/Laue_lab/Microscopy/HP1b_live_blinkingDyes/20221018_Halo-JF639b_Hoechst-SPY505'
load_data = T
partial_load = F
...
lapply(data_list, function(df) filter(df, Frame < 39982)) -> data_list #the weird last frames with stationary mol-s
names(data_list) = sapply(data_list, function(x) x$Pos[1])
...
#   data_pdist %>% mutate(FrameRound = ceiling(Frame/100)*100) %>% group_by(Pos, FrameRound) %>%
#     summarise(Mean_pdist_mean = mean(pdist_mean),
#               Min_pdist_mean = min(pdist_mean),
#               Mean_pdist_min = mean(pdist_min)) %>%
#     mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                     Pos == 'Pos6' | Pos == 'Pos7')) -> data_pdist_avg
...
#   ggsave('meanLocDist.png', width = 10, height = 4)
...
#     theme_bw() +
#     theme(aspect.ratio = 0.15, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('minLocDist.png', width = 10, height = 4)
#   ggplot(data_pdist_avg, aes(x = FrameRound, y = Mean_pdist_min/1000, colour = Pos)) +
#     geom_line() +
#     facet_grid(tag~., labeller = labeller(tag = c('TRUE' = 'Pos0-7', 'FALSE' = 'Pos8-14'))) +
...
#     theme_bw() +
#     theme(aspect.ratio = 0.15, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('minLocDist_zoom.png', width = 10, height = 4)
#   data_pdist %>% mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                                  Pos == 'Pos6' | Pos == 'Pos7')) -> data_pdist
...
#     theme_bw() +
#     theme(aspect.ratio = 0.15, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('minLocDist_cumul.png', width = 10, height = 4)
#   data %>% group_by(Pos, Frame) %>%
#     summarise(pdist = c(pdist(matrix(c(x, y), nrow=n())))[c(pdist(matrix(c(x, y), nrow=n())))!=0]) %>%
#     mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                     Pos == 'Pos6' | Pos == 'Pos7')) -> data_pdist_all
#   filter(data_pdist_all, pdist < 1000) %>% mutate(cycle = paste0('cycle', ceiling(Frame/10000)-1),
#                                                   Frame = Frame - (ceiling(Frame/10000)-1)*10000) %>%
#     ggplot(aes(x = pdist/1000, y = ave(..count.., group, FUN = cumsum),
#                colour = Pos, group = interaction(Pos, cycle))) +
#     geom_freqpoly(binwidth = 0.02) +
#     facet_grid(cycle~tag, scales = 'free_y',
#                labeller = labeller(tag = c('TRUE' = 'Pos0-7', 'FALSE' = 'Pos8-14'))) +
...
#   ggsave('locDist_cumulHist_1um.png', width = 8, height = 5.5)
#   filter(data_pdist_all, pdist < 500) %>% mutate(cycle = paste0('cycle', ceiling(Frame/10000)-1),
#                                                  Frame = Frame - (ceiling(Frame/10000)-1)*10000) %>%
#     ggplot(aes(x = pdist/1000, y = ave(..count.., group, FUN = cumsum),
#                colour = Pos, group = interaction(Pos, cycle))) +
...
#     theme_bw() +
#     theme(aspect.ratio = 0.4, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('locDist_cumulHist_500nm.png', width = 8, height = 5.5)
#   
#   filter(data_pdist_all, pdist < 500) %>% mutate(cycle = paste0('cycle', ceiling(Frame/5000)-1),
...
#     theme_bw() +
#     theme(aspect.ratio = 0.4, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('locDist_cumulHist_500nm_5000frCycles.png', width = 8, height = 8)
#   
#   #Number of localisations
#   data %>% group_by(Pos, Track) %>% summarise(Track_length = n()) %>% group_by(Pos) %>%
#     summarise(Num_points = sum(Track_length), Num_tracks = n()) -> data_numPoints
#   data_SMLM %>% group_by(Pos, Track) %>% summarise(Track_length = n()) %>% group_by(Pos) %>%
...
#   ggsave('numLoc.png', width = 4, height = 2.5)
...
#   ggarrange(numTrack1, numTrack2, nrow = 2)
#   ggsave('numTrack.png', width = 4, height = 5)
#   data %>% group_by(Pos, Track) %>% summarise(Track_length = n()) %>%
#     filter(Track_length > 1) %>% group_by(Pos, Track_length) %>%
...
#     group_by(Pos, Track_length) %>% summarise(Num_tracks = sum(Num_tracks)) -> data_numLenTr2
...
#   ggsave('numLenTrack.png', width = 5, height = 3)
#   data_SMLM %>% group_by(Pos, Track) %>% summarise(Track_length = n()) %>%
#     group_by(Pos, Track_length) %>% summarise(Num_tracks = n()) -> data_numLenTr_SMLM
#   data_numLenTr_SMLM %>% mutate(Track_length = case_when(Track_length > 5 ~ '>5',
#                                                          T ~ as.character(Track_length))) %>%
#     group_by(Pos, Track_length) %>% summarise(Num_tracks = sum(Num_tracks)) -> data_numLenTr_SMLM2
...
#     theme(aspect.ratio = 0.5, plot.subtitle = element_text(hjust = 0.5), plot.title = element_text(hjust = 0.5),
#           axis.text.x = element_text(angle = 30, vjust = 0.8))
#   ggsave('numLenTrack_SMLM.png', width = 5, height = 3)
#   data %>% mutate(FrameRound = ceiling(Frame/100)*100) %>% group_by(Pos, FrameRound) %>% summarise(Num_loc = n()/100) %>%
#     mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                     Pos == 'Pos6' | Pos == 'Pos7')) -> data_locPerFr
...
#     theme_bw() +
#     theme(aspect.ratio = 0.15, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('numLocFr.png', width = 10, height = 4)
#   data %>% mutate(FrameRound = ceiling(Frame/100)*100) %>% group_by(Pos, FrameRound) %>%
...
#     mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                     Pos == 'Pos6' | Pos == 'Pos7')) -> data_locPerFr
#   ggplot(data_locPerFr, aes(x = FrameRound, y = Num_loc, colour = Pos, group = interaction(Pos, cycle))) +
#     geom_line() +
#     facet_grid(cycle~tag, scale = 'free_y', labeller = labeller(tag = c('TRUE' = 'Pos0-7', 'FALSE' = 'Pos8-14'))) +
#     labs(x = 'Frame', y = 'Number of locs', title = 'Number of localisations per frame',
...
#   ggsave('numLocFr2.png', width = 8, height = 5.5)
# }
# 
# #Track lengths
# {
#   data %>% group_by(Pos, Track) %>% summarise(Track_length = n()) -> data_trLen
#   data_SMLM %>% group_by(Pos, Track) %>% summarise(Track_length = n()) -> data_trLen_SMLM
...
#   ggsave('TrackLen.png', width = 4, height = 5)
#   ggplot(data_trLen, aes(x = Track_length)) +
#     geom_histogram(binwidth = 1, aes(y = stat(width*density)), colour = 'gray20', fill = 'gray80') +
#     labs(title = 'Distribution of track lengths',
#          subtitle = 'All Pos\nTracking r=400 nm, no gaps (for diff)',
...
#   ggsave('TrackLen_longZoom.png', width = 4, height = 5)
#   
...
#   ggsave('TrackLen_perPos.png', width = 4, height = 6)
# }
# 
# #Jump distances
# {
#   ##QC check
#   data %>% filter(Pos == 'Pos0' & Frame > 22000 |
#                     Pos == 'Pos1' & Frame > 14000 |
#                     Pos == 'Pos2' & Frame > 10000 |
#                     Pos == 'Pos3' & Frame > 9000 |
#                     Pos == 'Pos4' & Frame > 10000 |
#                     Pos == 'Pos5' & Frame > 10000 |
#                     Pos == 'Pos6' & Frame > 9000 | #looks like the cell is a bit smaller than Pos8?
#                     Pos == 'Pos7' & Frame > 10000 | #looks like the cell is larger than Pos2,6,8?
#                     Pos == 'Pos8' & Frame > 10000 |
#                     Pos == 'Pos10' & Frame > 2000 |
#                     Pos == 'Pos11' & Frame > 15000 |
#                     Pos == 'Pos12' & Frame > 11500 |
#                     Pos == 'Pos13' & Frame > 10000 |
#                     Pos == 'Pos14' & Frame > 10000 |
#                     Pos == 'Pos9' & Frame > 6000) -> data_filtered #also edited based on reproducibility in "cycles"
#   data_filtered %>% group_by(Pos, Frame) %>% summarise(N = n()) %>%
#     mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                     Pos == 'Pos6' | Pos == 'Pos7')) %>%
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5)
#   ggsave('numLocFr_filtered.png', width = 5, height = 4.5)
#   data_filtered %>% group_by(Pos, Frame) %>%
#     summarise(pdist = c(pdist(matrix(c(x, y), nrow=n())))[c(pdist(matrix(c(x, y), nrow=n())))!=0]) -> data_pdist_all_filt
#   data_pdist_all_filt %>% mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                                           Pos == 'Pos6' | Pos == 'Pos7')) %>%
#     filter(pdist < 500) %>%
#     ggplot(aes(x = pdist/1000, y = ave(..count.., group, FUN = cumsum),
#                colour = Pos, group = Pos)) +
#     geom_freqpoly(binwidth = 0.01) +
#     #facet_grid(tag~., labeller = labeller(tag = c('TRUE' = 'Pos0-7', 'FALSE' = 'Pos8-14'))) +
#     scale_x_continuous(breaks = seq(0, 1, by = 0.1), minor_breaks = seq(0, 1, by = 0.05)) +
...
#     theme_bw() +
#     theme(aspect.ratio = 0.4, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('locDist_cumulHist_500nm_filtered.png', width = 5.5, height = 4)
#   data_pdist_all_filt %>% mutate(tag = (Pos == 'Pos0' | Pos == 'Pos1' | Pos == 'Pos2' | Pos == 'Pos3' | Pos == 'Pos4' | Pos == 'Pos5' |
#                                           Pos == 'Pos6' | Pos == 'Pos7')) %>%
#     ggplot(aes(x = pdist/1000, colour = Pos, group = Pos)) +
#     geom_density(stat = 'ecdf') +
#     facet_grid(tag~., labeller = labeller(tag = c('TRUE' = 'Pos0-7', 'FALSE' = 'Pos8-14'))) +
...
#     theme_bw() +
#     theme(aspect.ratio = 0.4, plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
#   ggsave('locDist_cumul_filtered.png', width = 5.5, height = 4.5)
#   
#   #Overall JD
#   data_filtered %>%
#     filter(Track_length == 2) %>% group_by(Pos, Track) %>%
...
#   ggsave('JumpDist.png', width = 3, height = 2.5)
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   ggsave('JumpDist_perPos.png', width = 4, height = 4)
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   ggsave('JumpDist_perPosCycle1.png', width = 4, height = 4.5)
#   mutate(jumpDist, cycle = paste0('cycle', ceiling(Frame1/10000)-1)) %>%
#     ggplot(aes(x = disp, colour = Pos, linetype = cycle)) +
#     geom_density() +
#     facet_grid(Pos~.) +
...
#   ggsave('JumpDist_perPosCycle2.png', width = 4, height = 12)
#   
#   #JD with increasing interval
#   for (i in 1:8) {
#     data_filtered %>% filter(Track_length > i) %>% group_by(Pos, Track) %>%
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   ggsave('JumpDist_intervals.png', width = 4.5, height = 3)
#   
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   ggsave('JumpDist_trLen.png', width = 4.5, height = 3)
#   
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   #ggsave('JumpDist_intervals_longTraj.png', width = 5, height = 3)
#   
#   filter(jumpDist_1234, Track_length < 15) %>% #looks much more reasonable -> use this for MSD
#     ggplot(aes(x = disp, colour = factor(interval*3, levels = c('3', '6', '9', '12',
#                                                                 '15', '18', '21', '24')))) +
#     geom_density() +
#     scale_colour_manual(values = gray.colors(8, 0.2, 0.75)) +
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 0.5, axis.title.y = element_blank())
#   ggsave('JumpDist_intervals_shortTraj.png', width = 4.5, height = 3)
#   
#   #MSD
#   filter(jumpDist_1234, Track_length < 15) %>% group_by(interval) %>% summarise(MSD = mean(disp^2)/1000000) -> MSD
#   filter(jumpDist_1234, Track_length < 15) %>% group_by(Pos, interval) %>% summarise(MSD = mean(disp^2)/1000000) -> MSD_perPos
#   ggplot(MSD, aes(x = interval*3, y = MSD)) +
#     geom_point() +
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD.png', width = 3, height = 3)
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_perPos.png', width = 4, height = 4)
#   ggplot(MSD, aes(x = log10(interval*3/1000), y = log10(MSD))) +
#     geom_point() +
#     labs(title = 'MSD curve', subtitle = 'All Pos, dens < 10 loc/fr, traj <15',
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_loglog.png', width = 3, height = 3)
#   lm(log10(MSD)~log10(interval*3/1000), data = MSD) -> MSDfit
#   ggplot(MSD, aes(x = log10(interval*3/1000), y = log10(MSD))) +
#     geom_point() +
#     geom_abline(intercept = MSDfit$coefficients[[1]], slope = MSDfit$coefficients[[2]]) +
#     annotate('text', x = -1.9, y = -1.8,
#              label = paste('D =', round(10^MSDfit$coefficients[[1]], 2),
#                            'um2/s,\nalpha =', round(MSDfit$coefficients[[2]], 2))) +
#     labs(title = 'MSD curve', subtitle = 'All Pos, dens < 10 loc/fr, traj < 15',
#          x = 'Log(Interval, s)', y = 'Log(MSD, um2)') +
#     theme_bw() +
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_loglogfit.png', width = 3, height = 3)
#   split(MSD_perPos, MSD_perPos$Pos) %>% lapply(function(x) {
#     lm(log10(MSD)~log10(interval*3/1000), data = x)
#   }) -> MSDfit_perPos.list
#   MSDfit_perPos.df = data.frame(Pos = names(MSDfit_perPos.list),
...
#   ggplot(MSD_perPos, aes(x = log10(interval*3/1000), y = log10(MSD), colour = Pos)) +
#     geom_point() +
#     #geom_line(size = 0.2) +
#     geom_abline(data = MSDfit_perPos.df, aes(intercept = Intercept, slope = Slope, colour = Pos)) +
...
#     scale_colour_discrete(labels = paste0(MSDfit_perPos.df$Pos, '\n', MSDfit_perPos.df$label)) +
#     labs(title = 'MSD curve', subtitle = 'Dens < 10 loc/fr, traj <15',
#          x = 'Log(Interval, s)', y = 'Log(MSD, um2)') +
#     theme_bw() +
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_perPos_loglog.png', width = 5.5, height = 4.5)
#   ggplot(MSDfit_perPos.df, aes(x = '', y = 10^Intercept)) +
#     geom_boxplot(width = 0.2) +
#     geom_jitter(width = 0.05, aes(colour = Pos)) +
#     labs(x = '', y = 'D, um2/s') +
#     theme_bw() +
#     theme(aspect.ratio = 3, legend.position = 'none') -> Ds
#   ggplot(MSDfit_perPos.df, aes(x = '', y = Slope)) +
#     geom_boxplot(width = 0.2) +
#     geom_jitter(width = 0.05, aes(colour = Pos)) +
#     labs(x = '', y = 'Alpha') +
#     theme_bw() +
...
#   filter(MSD, interval < 6) %>% 
#     lm(log10(MSD)~log10(interval*3/1000), data = .) -> MSDfit2
#   ggplot(MSD, aes(x = log10(interval*3/1000), y = log10(MSD), shape = interval<6)) +
#     geom_point() +
#     scale_shape_manual(values = c(4, 19), guide = 'none') +
#     geom_abline(intercept = MSDfit2$coefficients[[1]], slope = MSDfit2$coefficients[[2]]) +
#     annotate('text', x = -1.9, y = -1.8,
#              label = paste('D =', round(10^MSDfit2$coefficients[[1]], 2),
#                            'um2/s,\nalpha =', round(MSDfit2$coefficients[[2]], 2))) +
#     labs(title = 'MSD curve', subtitle = 'All Pos, dens < 10 loc/fr, traj < 15',
#          x = 'Log(Interval, s)', y = 'Log(MSD, um2)') +
#     theme_bw() +
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_loglog2.png', width = 3, height = 3)
#   split(filter(MSD_perPos, interval < 6), filter(MSD_perPos, interval < 6)$Pos) %>% lapply(function(x) {
#     lm(log10(MSD)~log10(interval*3/1000), data = x)
#   }) -> MSDfit_perPos2.list
#   MSDfit_perPos2.df = data.frame(Pos = names(MSDfit_perPos2.list),
...
#   ggplot(MSD_perPos, aes(x = log10(interval*3/1000), y = log10(MSD), colour = Pos, shape = interval<6)) +
#     geom_point() +
#     scale_shape_manual(values = c(4, 19), guide = 'none') +
#     geom_abline(data = MSDfit_perPos2.df, aes(intercept = Intercept, slope = Slope, colour = Pos)) +
#     # geom_text(data = MSDfit_perPos.df,
#     #           aes(x = rep(-2.4, 6), y = seq(-1.3, -1.8, length.out = 6),
#     #               label = label, colour = Pos)) +
#     # annotate('text', x = -1.9, y = -1.8,
#     #          label = paste('D =', round(10^MSDfit.df$Intercept[[1]], 2),
#     #                        'um2/s,\nalpha =', round(MSDfit.df$Slope[[1]], 2))) +
#     scale_colour_discrete(labels = paste0(MSDfit_perPos2.df$Pos, '\n', MSDfit_perPos2.df$label)) +
#     labs(title = 'MSD curve', subtitle = 'Dens < 10 loc/fr, traj <15',
#          x = 'Log(Interval, s)', y = 'Log(MSD, um2)') +
#     theme_bw() +
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('MSD_perPos_loglog2.png', width = 5.5, height = 4.5)
#   ggplot(MSDfit_perPos2.df, aes(x = '', y = 10^Intercept)) +
#     geom_boxplot(width = 0.2) +
#     geom_jitter(width = 0.05, aes(colour = Pos)) +
#     labs(x = '', y = 'D, um2/s') +
#     theme_bw() +
#     theme(aspect.ratio = 3, legend.position = 'none') -> Ds
#   ggplot(MSDfit_perPos2.df, aes(x = '', y = Slope)) +
#     geom_boxplot(width = 0.2) +
#     geom_jitter(width = 0.05, aes(colour = Pos)) +
#     labs(x = '', y = 'Alpha') +
#     theme_bw() +
...
#   filter(jumpDist_1234, interval == 1) %>% group_by(Pos, Track) -> jumpDist_1
#   avcorr = data.frame(lag = 0:6, avcorr_x = rep(NA, 7), avcorr_y = rep(NA, 7), avcorr = rep(NA, 7))
#   for (lag in 0:6) {
#     jumpDist_1 %>% group_by(Pos, Track) %>%
#       mutate(corr_x = diff.x*lag(diff.x, lag),
#              corr_y = diff.y*lag(diff.y, lag)) %>% drop_na() %>%
#       summarise(trcorr_x = mean(corr_x, na.rm = T),
#                 trcorr_y = mean(corr_y, na.rm = T)) %>%
#       ungroup() %>% summarise(avcorr_x = mean(trcorr_x), avcorr_y = mean(trcorr_y)) %>%
...
#   }
#   ggplot(avcorr, aes(x = lag, y = avcorr/1000000)) +
...
#          title = 'Autocovariance', subtitle = 'Solid = 2D, dashed = 1D') +
#     theme_bw() +
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('Autocovariance.png', width = 3, height = 3)
# }
...
thresholds1 = c(1500, 800, 650, 200, 200, 200, 50, 100, 700, 600, 800, 500, 100, 1200, 400)
thresholds2 = c(22000, 14000, 10000, 9000, 10000, 10000, 2000, 15000, 11500, 10000, 10000, 6000, 9000, 10000, 10000)
n_shuffle = 6
segmCoresList = list()
for (c in 1:length(data_list)) {
  ...
  #Calculate result
  {
    filter(data_list_SMLM[[c]], Frame > thresholds1[c]) %>% group_by(Pos, Track) %>%
    ...
    filter(data_list[[c]], Frame > thresholds2[c]) %>%
      filter(Track_length > 1 & Track_length < 8) %>% group_by(Pos, Track) %>%
  ...
  {
    eps = 50
    #minPts = round(1200000000/mean(filter(data_pdist, Pos == data_list[[c]]$Pos[[1]])$pdist_mean)^2)
    ...
    ggplot(data_SMLM_all, aes(x = x, y = y, colour = cluster!=0)) +#colour = interaction(cluster!=0, cluster_core))) +
    ...
    # ggplot(data_SMLM_all, aes(x = x, y = y, colour = cluster!=0)) +#colour = interaction(cluster!=0, cluster_core))) +
    #   geom_point(size = 0.3, shape = 20, stroke = 0) +
    #   scale_colour_manual(values = c('gray', '#e1ad01ff'),
    #                       labels = c('Outside', 'Cluster')) +
    #   coord_fixed() +
    #   labs(colour = '', title = 'DBSCAN segmentation',
    #        subtitle = paste0('Eps = ', eps, ' nm, minPts = ', minPts, '\nAll data (frame>', thresholds1[c], '); r=300 nm, <3 gaps')) +
    #   theme_bw() +
    #   theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))# -> dbscan_segm
    # ggsave('Pos1_DBSCAN_Y.png', width = 5, height = 5, dpi = 500)
    ...
    segmentation = list('cluster' = clust_all$cluster,
                        'eps' = 50, 'minPts' = 0, metric = 'euclidean', borderPoints = T)
  ...
  ggsave(file = file.path(plot_dir, paste0(data_list[[c]]$Pos[[1]], '.png')),
...
##Plot JD
Nfoci = nrow(filter(data, Focus_group == 'Focus'))
filter(data, Focus_group != 'Focus') %>% group_by(Focus_group) %>% summarise(N = n()) -> Npfoci
ggplot(data, aes(x = disp, colour = Focus_group, size = Focus_group)) +
  ...
  labs(title = 'Jump distance distribution', x = 'Displacement, nm',
       subtitle = paste0('All Pos, dens < 10 loc/fr (r=400 nm, no gaps)\nTraj of l = 2\n',
  ...
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
        aspect.ratio = 0.5, axis.title.y = element_blank())
ggsave('New_analysis/JumpDist_segm_dbscan.png', width = 4.5, height = 3)
  ...
  for (pos in poses) {
    for (i in 1:WFframes) {
      ...
      png = readPNG(file.path('TopHat_segmentation/PNGs_ManThreshold_coloured/',
                              paste0(pos, '_foci', paste0(rep(0, 2-nchar(as.character(i-1))), collapse = ''), i-1, '.png')))
    ...
    tmp_df %>% group_by(R, G, B) %>% mutate(focus_id = cur_group_id()) %>% ungroup() %>% select(-R, -G, -B) %>%
    ...
    # }
    foci_list[[pos]] = tmp_df
    pfoci_list[[pos]] = tmp_df2
  }
...
if (classify) {
  lapply(names(data_list), function(pos) {
    ...
    left_join(df, foci_list[[pos]], by = c('px_id', 'frame')) %>%
      ...
      select(-x_left, -y_top, -dim_y, -dim_x, -pos) %>% rename(DNA_focus_idx = focus_id) %>%
    ...
    left_join(result, pfoci_list[[pos]], by = c('px_id', 'frame')) %>%
      ...
      select(-x_left, -y_top, -dim_y, -dim_x, -frame, -pos) %>%
      mutate(pseudofocus = (replace_na(pseudofocus, 0) != 0)) %>%
      rename(DNA_pseudo = pseudofocus, DNA_px_id = px_id) -> result#%>%
...
# #Sanity check
# ggplot(data_list[[5]], aes(x = x, y = y, colour = as.character(DNA_focus_idx))) +
#   geom_point(size = 0.1) +
#   scale_y_reverse() +
#   coord_fixed() +
#   theme_bw()
# ggplot(data_list[[5]], aes(x = x, y = y, colour = Track, group = Track)) +
#   geom_point(size = 0.1) +
#   geom_path() +
#   scale_y_reverse() +
#   coord_fixed() +
...
lapply(seq_along(data_list), function(c) {
  ...
  df %>% group_by(Pos, Track) %>% filter(n() == 2) %>%
...
split(jumpDist2, jumpDist2$Pos) -> jumpDist2_list
...
ggplot(jumpDist2, aes(x = disp, colour = interaction((DNA_focus_idx != 0), DNA_pseudo != 0))) +
  ...
  labs(title = 'Jump distance', x = 'Displacement, nm', y = '', colour = '',
       subtitle = paste0('Foci: ', sum(jumpDist2$DNA_focus_idx!=0),
                         ' jumps; pseudo: ', sum(jumpDist2$DNA_pseudo), ' jumps')) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), aspect.ratio = 0.5, axis.title.y = element_blank(),
        plot.subtitle = element_text(hjust = 0.5))
ggsave('New_analysis/JumpDist_EuHet.png', width = 4.7, height = 3)
...
#     geom_point(size = 0.3, shape = 20, stroke = 0) +
#     scale_y_reverse() +
#     scale_colour_manual(values = c('gray', brewer_pal(palette = 'Set2')(length(unique(data_list[[c]]$DNA_focus_idx))-1))) +
#     coord_fixed() +
#     theme_bw() +
...
#       select(-x_left, -y_top, -dim_y, -dim_x, -pos) %>% rename(DNA_focus_idx = focus_id) %>%
#       mutate(DNA_focus_idx = replace_na(DNA_focus_idx, 0),
#              DNA_focus = case_when(DNA_focus_idx == 0 ~ F, T ~ T)) -> data_SMLM_all
...
#       loops = 0
#       while(is.bad) {
#         filter(ungroup(data_SMLM_all), DNA_focus) %>%
...
#                                                             newdata = select(ungroup(data_SMLM_all), x, y),
#                                                             data = select(ungroup(pseudoclust_all), x, y))
#       # ggplot(data_SMLM_all, aes(x = x, y = y, colour = interaction(DNA_focus, Pseudo_in_DNA1 != 0))) +
#       #   geom_point(size = 0.1) +
  ...
  lapply(data_list, function(dat) {
    dat %>% group_by(HP1_foci_dbscan) %>%
      mutate(DNA_HP1_focus = case_when(HP1_foci_dbscan == 0 ~ F,
                                       ...
                                       T ~ F)) %>% ungroup() -> result
    # ggplot(result, aes(x = x, y = y, colour = interaction(DNA_focus, HP1_foci_dbscan != 0))) +
...
##Jump distance analysis - not included in the paper
{
  ...
  #               Pos, Track, Frame, x, y, Track_length)
  #   
  ...
  #     select(-x_left, -y_top, -dim_y, -dim_x, -pos) %>%
  #     left_join(., pfoci_list[[pos]], by = c('px_id', 'frame')) %>%
  #     select(-x_left, -y_top, -dim_y, -dim_x, -pos, -frame, -px_id) %>%
  #     rename(DNA_focus_idx = focus_id, DNA_pseudo = pseudofocus) %>%
  #     mutate(DNA_focus_idx = replace_na(DNA_focus_idx, 0),
  #            DNA_pseudo = replace_na(DNA_pseudo, 0)) -> result
  #   
  ...
  #                                     T ~ 'Euchromatin')) %>%
  #     group_by(Compartment2) %>% summarise(num_obs = n()) %>%
  ...
  #   ggsave('New_analysis/jumpDist_segm_HP1andDNA_numObs.png', width = 4, height = 3)
  # }
  # #Just the real foci
  # {
  ...
  #   theme_bw() +
  #   theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), aspect.ratio = 0.5)
  # ggsave('New_analysis/jumpDist_segm_HP1andDNA_allCompartments.png', width = 4.7, height = 3)
  # jumpDist3 %>%
  #   mutate(Compartment = case_when(DNA_HP1_foci ~ 'DNA&HP1-high',
  #                                  DNA_focus_idx!=0 & !DNA_HP1_foci ~ 'DNA-high',
  #                                  !DNA_HP1_foci & HP1_foci_dbscan!=0 ~ 'HP1-high',
  ...
  #   theme_bw() +
  #   theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), aspect.ratio = 0.5)
  # ggsave('New_analysis/jumpDist_segm_HP1andDNA_allCompartments2.png', width = 5, height = 3)
  # jumpDist3 %>%
  #   mutate(Compartment = case_when(DNA_HP1_foci ~ 'DNA&HP1-high',
  #                                  (Pseudo_in_DNA1 | Pseudo_in_DNA2 | Pseudo_in_DNA3 |
  #                                     Pseudo_in_DNA4 | Pseudo_in_DNA5 | Pseudo_in_DNA6) &
...
#     print(i)
#     #Calculate jump dist
#     df %>% filter(Track_length > i) %>% group_by(Pos, Track) %>%
#       filter(n() > i) %>% #because initial filtering by frame cuts some trajectories up
#       mutate(diff.x = c(rep(NA, i), diff(x, i)), diff.y = c(rep(NA, i), diff(y, i)),
...
#   
#   mutate(MSD_segm, colour = case_when(grepl('HP1 pseudo', Compartment2) & !grepl('DNA', Compartment2) ~ 'HP1 pseudo',
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('New_analysis/MSD_HP1andDNA.png', width = 5.5, height = 4)
#   
#   filter(MSD_segm, !grepl('pseudo', Compartment2)) %>%
...
#     theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),
#           aspect.ratio = 1)
#   ggsave('New_analysis/MSD_HP1andDNA_loglogLine.png', width = 5.5, height = 4)
#   }
#   
...
#   mutate(MSDfit_segm.df, label = paste('D =', round(10^Intercept, 2), 'um2/s, alpha =', round(Slope, 2), '\n')) -> MSDfit_segm.df
#   MSDfit_segm.df %>%
#     mutate(Compartment2 = factor(Compartment2, levels = c('DNA-high', 'HP1-high', 'DNA&HP1-high',
#                                                           'DNA pseudo', 'HP1 pseudo', 'DNA&HP1 pseudo',
...
#   
#   ggplot(MSDfit_segm.df, aes(x = Compartment2, y = Intercept, colour = Compartment2, shape = Compartment2)) +
#     geom_point() +
...
#   mutate(MSDfit_segm.df2, label = paste('D =', round(10^Intercept, 2), 'um2/s, alpha =', round(Slope, 2), '\n')) -> MSDfit_segm.df2
#   MSDfit_segm.df2 %>%
#     mutate(Compartment2 = factor(Compartment2, levels = c('DNA-high', 'HP1-high', 'DNA&HP1-high',
#                                                           'DNA pseudo', 'HP1 pseudo', 'DNA&HP1 pseudo',
...
#   
#   ggplot(MSDfit_segm.df2, aes(x = Compartment2, y = Intercept, colour = Compartment2, shape = Compartment2)) +
#     geom_point() +
#     scale_y_continuous(labels = math_format(10^.x),
#                        sec.axis = sec_axis(trans=~10^., breaks = c(0.02, 0.05, 0.1, 0.2, 0.5, 1, 2))) +
#     scale_colour_manual(values = c('DNA-high' = 'cyan', 'HP1-high' = 'red', 'DNA&HP1-high' = 'purple',
...
#                                    'Euchromatin' = 'gray')) +
#     scale_shape_manual(values = c(19, 19, 19, 4, 4, 4, 19)) +
#     geom_errorbar(data = MSDfit_segm_bs_stats, aes(ymin = int.CI_l, ymax = int.CI_h,
#                                                 y = NULL, alpha = NULL),
#                   width = 0.1, size = 0.3) +
#     labs(x = '', y = 'D, um2/s') +
#     theme_bw() +
...
#=> reclassify each *JUMP* by compartment (if needed, duplicate the localisation);
...
lapply(seq_along(data_list), function(c) {
  ...
  dim_x = foci_list[[pos]]$dim_x[1]
  dim_y = foci_list[[pos]]$dim_y[1]
  df %>% group_by(Pos, Track) %>% filter(Track_length > 1) %>%
  ...
  result %>%
   ...
   mutate(Compartment1 = lead(Compartment2)) %>%
    mutate(Compartment1 = case_when(is.na(Compartment1) ~ 'End',
                                    T ~ Compartment1)) %>%
    select(Pos, Track, Frame, x, y, Track_length, Compartment1, Compartment2) -> result
...
lapply(seq_along(data_list), function(c) {
  ...
  result %>% group_by(Track) %>%
    ...
    select(Pos, Track, Frame, x, y, Track_length, Compartment) -> result
...
#Smallest sample: DNA&HP1-high - 5,078 traj
...
data %>% filter(Pos == 'Pos0' & Frame > 22000 |
                  ...
                  Pos == 'Pos13' & Frame > 10000 |
                  Pos == 'Pos14' & Frame > 10000 |
                  Pos == 'Pos9' & Frame > 6000) -> data_filtered
...
##Calculate density of HP1 in HP1 foci (DNA-rich and DNA-poor) and HP1 pseudofoci (SMLM data)
{
  ...
  }) %>% bind_rows(.id = 'Pos') -> HP1_foci_dens_raw
  ...
  HP1_foci_dens %>% mutate(Pos = renamer[Pos]) -> HP1_foci_dens
  ...
  HP1_foci_dens_raw %>% rename(NNdist_mean_raw = NNdist_mean_focus, density_raw = density) %>%
    ...
    left_join(rename(HP1_pfoci_dens, NNdist_mean_pseudo1 = NNdist_mean1, density_pseudo1 = density1,
              ...
              by = c('Pos', 'cluster')) %>% 
    left_join(DNA_HP1_foci.df, by = c('Pos', 'cluster')) %>%
...
##Summarise and plot HP1 density in foci
{
  ...
  pivot_longer(HP1_all_dens, starts_with('ratio'), names_to = 'Category', values_to = 'ratio') %>%
    ...
    scale_colour_manual(values = c('red', 'orange'),
                        labels = c('Foci/Pseudofoci', 'Foci cores/Pseudofoci')) +
    coord_cartesian(xlim = c(0, 10)) +
...
{
  # data_filtered %>% filter(HP1_foci_dbscan != 0) %>% group_by(Pos, HP1_foci_dbscan) %>%
  #   summarise(NNdist_mean_focus = 0.001*mean(kNNdist(matrix(c(x, y), nrow=n()), k = 1, all = F))) %>%
  #   mutate(density = 1/(4*NNdist_mean_focus^2)) -> HP1_foci_dens
  # 
  # data_filtered %>% filter(HP1_pseudo_dbscan1 != 0) %>% group_by(Pos, HP1_pseudo_dbscan1) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # data_filtered %>% filter(HP1_pseudo_dbscan2 != 0) %>% group_by(Pos, HP1_pseudo_dbscan2) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # data_filtered %>% filter(HP1_pseudo_dbscan3 != 0) %>% group_by(Pos, HP1_pseudo_dbscan3) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # data_filtered %>% filter(HP1_pseudo_dbscan4 != 0) %>% group_by(Pos, HP1_pseudo_dbscan4) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # data_filtered %>% filter(HP1_pseudo_dbscan5 != 0) %>% group_by(Pos, HP1_pseudo_dbscan5) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # data_filtered %>% filter(HP1_pseudo_dbscan6 != 0) %>% group_by(Pos, HP1_pseudo_dbscan6) %>%
  #   filter(n() > 1) %>% #cluster of size 1 gives an error in kNNdist
  ...
  # 
  # left_join(HP1_foci_dens, HP1_pseudo_dens1, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan1')) %>%
  #   left_join(HP1_pseudo_dens2, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan2')) %>%
  #   left_join(HP1_pseudo_dens3, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan3')) %>%
  #   left_join(HP1_pseudo_dens4, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan4')) %>%
  #   left_join(HP1_pseudo_dens5, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan5')) %>%
  #   left_join(HP1_pseudo_dens6, by = c('Pos' = 'Pos', 'HP1_foci_dbscan' = 'HP1_pseudo_dbscan6')) %>%
  #   rename(cluster = HP1_foci_dbscan) %>% left_join(DNA_HP1_foci.df, by = c('Pos', 'cluster')) %>%
  #   mutate(is_DNA = replace_na(is_DNA, F)) -> HP1_all_dens
# 
# ##Plot stuff
#   ##Density histograms
...
##Calculate density of HP1 in DNA foci (HP1-rich and HP1-poor) and pseudofoci (SMLM data)
{
  ...
  for (c in seq_along(data_list_SMLM)) {
    ...
    print(pos)
    filter(data_list_SMLM[[c]], Frame > thresholds1[c]) %>% group_by(Pos, Track) %>%
      summarise(x = mean(x), y = mean(y), Frame = mean(Frame), Pos = Pos[1]) %>% ungroup() %>%
      select(Pos, Frame, x, y) -> df
...
#   tmp1 %>% group_by(Pos, DNA_focus_idx) %>%
#     summarise(NNdist_mean_focus = 0.001*mean(kNNdist(matrix(c(x, y), nrow=n()), k = 1, all = F))) %>%
#     mutate(density_focus1 = 1/(4*NNdist_mean_focus^2)) -> DNA_foci_dens
#   filter(tmp1, HP1_foci_dbscan == 0) %>% group_by(Pos, DNA_focus_idx) %>%
#     summarise(NNdist_mean_focus_noHP1 = 0.001*mean(kNNdist(matrix(c(x, y), nrow=n()), k = 1, all = F))) %>%
#     mutate(density_focus1_noHP1 = 1/(4*NNdist_mean_focus_noHP1^2)) %>%
#     right_join(DNA_foci_dens, by = c('Pos', 'DNA_focus_idx')) -> DNA_foci_dens
#   
#   tmp1 %>% group_by(Pos, DNA_focus_idx) %>% summarise(num_obs = n()) %>%
#     left_join(group_by(bind_rows(foci_list, .id = 'Pos'), Pos, focus_id) %>% summarise(num_px = n()/WFframes),
#               by = c('Pos' = 'Pos', 'DNA_focus_idx' = 'focus_id')) %>%
#     mutate(density_focus2 = num_obs/(num_px*(pixel/subsampling/1000)^2)) %>%
#     right_join(DNA_foci_dens, by = c('Pos', 'DNA_focus_idx')) -> DNA_foci_dens
#   filter(tmp1, HP1_foci_dbscan == 0) %>% group_by(Pos, DNA_focus_idx) %>% summarise(num_obs_noHP1 = n()) %>%
#     right_join(DNA_foci_dens, by = c('Pos', 'DNA_focus_idx')) %>%
...
#   tmp2 %>% group_by(Pos) %>% summarise(num_obs = n()) %>%
#     left_join(group_by(bind_rows(pfoci_list, .id = 'Pos'), Pos) %>%
#                        summarise(num_px = n()/WFframes), by = 'Pos') %>%
#     mutate(density_pseudo = num_obs/(num_px*(pixel/subsampling/1000)^2)) -> DNA_pseudo_dens
#   filter(tmp2, HP1_foci_dbscan == 0) %>% group_by(Pos) %>% summarise(num_obs_noHP1 = n()) %>%
#     right_join(DNA_pseudo_dens, by = 'Pos') %>%
